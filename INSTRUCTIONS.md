...
### 1. Documentation and Hosting (IMPORTANT)
...
**Clustering requirement:** If you cluster posts by topic, your dashboard must expose the number of clusters as a tunable parameter. We will test what happens at the extremes: the results must be coherent and your UI must not break.
...
### 4. Unique Features (optional)

Past applicants have built the following features that resulted in successful outcomes:

- Topic model embeddings visualized using [Tensorflow Projector](https://projector.tensorflow.org/), [Datamapplot](https://github.com/TutteInstitute/datamapplot), or [Nomic](https://atlas.nomic.ai/)
- GenAI summaries of time-series plots for non-technical audiences
- Chatbot that refines and suggests follow-up queries based on the user's original query
- Connecting offline events (e.g. from Wikipedia or news APIs) with online post sharing for specific searches
- Connecting multiple platform datasets together to search across social platforms
- Semantic search after retrieving all posts matching a URL, so results can be queried beyond keyword matching
